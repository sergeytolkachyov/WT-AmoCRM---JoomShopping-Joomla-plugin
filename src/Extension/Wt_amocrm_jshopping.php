...
class Wt_amocrm_jshopping extends CMSPlugin
{
	 ...
	 * @since 1.0.0
	 */
	public function sendOrderToAmocrm($orderId)
	{
		...
		if (is_countable($amo_to_jshopping_fields) && count($amo_to_jshopping_fields) > 0)
		{
			foreach ($amo_to_jshopping_fields as $key => $value)
			{
				...
				foreach ($value->storefield as $jshopping_field)
				{
					...
					{// Получаем код купона
						$jshopping_field_value .= (string) $this->getCouponCode($order->$value) . " ";
					}
					elseif ($value == "shipping_method_id")
					{//название способа доставки
						$jshopping_field_value .= (string) $order->getShippingName() . " ";
					}
					elseif ($value == "payment_method_id")
					{//название способа оплаты
						$jshopping_field_value .= (string) $order->getPaymentName() . " ";
					}
					elseif ($value == "order_status")
					{//название статуса заказа
						$jshopping_field_value .= (string) $order->getStatus() . " ";
					}
...
//		$contact_id = $result[0]->contact_id;
	 ...
	 * @since 1.0.0
	 */
	private function getCouponCode($coupon_id) : string
	{
		$db    = Factory::getContainer()->get('DatabaseDriver');
		$query = $db->getQuery(true);
	...
	{
		if($this->params->get('use_utm_js_script', 1) == 1){
			...
			$wa->registerAndUseScript('plg_system_wt_amocrm_jshopping.wt_amocrm_jshopping_utm', 'plg_system_wt_amocrm_jshopping/wt_amocrm_jshopping_utm.js', array('version' => 'auto', 'relative' => true));
		}
	...
	public function onBeforeDisplayCheckoutFinish(&$text, $order_id)
	{
		...
		if ($this->params->get('debug') == 1)
		{
			...
			echo "<h3>WT Amo CRM - JoomShopping debug information</h3><br/>" . $debug_info;
